...
import load_copernicus_LAI as lai

sys.path.append('/home/dmilodow/DataStore_DTM/BALI/CARDAMOM_BALI/CARDAMOM_DTM/src/setup/')
...
for pp in range(0,len(plot)):
    ...
    LAI_unc_in = np.zeros(N_t)*np.nan
    """
    ...
    """
    # LAI data
    lat = latitude[plot_coord==plot[pp]]
    lon = longitude[plot_coord==plot[pp]]
    LAI_date,LAI,LAI_unc,tempQ = lai.load_point_LAI_nearest(lat,lon,np.datetime_as_string(start),np.datetime_as_string(end))
    #LAI_date, LAI, LAI_std, LAI_serr = field.get_LAI_ts(LAI_file,plot[pp])
    N_LAI = LAI_date.size
    for tt in range(0,N_LAI):
        LAI_in[date==LAI_date[tt]] = LAI[tt]
        LAI_unc_in[date==LAI_date[tt]] = LAI_unc[tt]
        """
    ...
    LAI_unc_in[np.isnan(LAI_unc_in)]=-9999.
    ...
    obs[obs_h=='LAI',:]=LAI_in.copy()
    obs[obs_h=='LAI_u',:]=LAI_unc_in.copy()
