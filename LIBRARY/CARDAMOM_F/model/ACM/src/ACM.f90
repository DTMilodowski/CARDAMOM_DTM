...
double precision, parameter :: xacc = 0.0001d0    & ! accuracy parameter for zbrent bisection proceedure ! 0.0001
                              ,dble_zero = 0d0    &
                              ,dble_one = 1d0     &
                              ,vsmall = tiny(0d0)
...
double precision, parameter :: &
          ...
          root_mass_length_coef_1 = (root_cross_sec_area * root_density)**(-1.d0), &
               const_sfc_pressure = 101325d0,      & ! (Pa)  Atmospheric surface pressure
                   ...
                   minlwp_default = -2.060814d0      ! min leaf water potential (MPa)
! structural parameters
double precision, parameter :: &
                   leaf_dimension = 0.08d0,        & ! length of leaf in characteristic dimension (m)
...
double precision, parameter :: &
                ...
                seconds_per_day_1 = 1d0/86400d0      ! inverse of seconds per day
...
!
  subroutine CARBON_MODEL(start,finish,met,pars,deltat,nodays,lat_in,lai_out,NEE_out,FLUXES,POOLS &
    ...
    ! the Aggregated Canopy Model for EvaporTranspiration (ACM_ET) to simulate potential evapotranspiration.
                     ...
                     ,deltaWP & ! deltaWP (MPa) minlwp-soilWP
                        ,Rtot & ! Total hydraulic resistance (MPa.s-1.m-2.mmol-1)
                         ,lat & ! latitude in degrees
                          ,ET & ! Total evapotranspiration (kgH2O/m2/day)
              ,wetcanopy_evap & ! kgH2O.m-2.day-1
               ,transpiration & ! kgH2O.m-2.day-1
                    ,soilevap   ! kgH2O.m-2.day-1
    ...
    max_lai_lwrad_absorption  = pars(6)   ! Max fraction of LW from sky absorbed by canopy
    lai_half_lwrad_absorption = pars(7)   ! LAI at which canopy LW absorption = 50 %
    max_lai_nir_absorption    = pars(8)   ! Max fraction of NIR absorbed by canopy
    lai_half_nir_absorption   = pars(9)   ! LAI at which canopy NIR absorption = 50 %
    minlwp                    = pars(10)  ! minimum leaf water potential (MPa)
    max_lai_par_absorption    = pars(11)  ! Max fraction of PAR absorbed by canopy
    lai_half_par_absorption   = pars(12)  ! LAI at which canopy PAR absorption = 50 %
    lai_half_lwrad_to_sky     = pars(13)  ! LAI at which 50 % LW is reflected back to sky
    ...
    max_lai_swrad_reflected   = pars(16)  ! Max fraction of SW reflected back to sky
    lai_half_swrad_reflected  = (lai_half_nir_absorption+lai_half_par_absorption) * 0.5d0
    max_lai_lwrad_release     = pars(17)  ! Max fraction of LW emitted from canopy to be released
    lai_half_lwrad_release    = pars(18)  ! LAI at which LW emitted from canopy to be released at 50 %
    soilevap_rad_intercept    = pars(19)  ! Intercept (kgH2O/m2/day) on linear adjustment to soil evaporation
                                          ! to account for non-calculation of energy balance
    soilevap_rad_coef         = pars(20)  ! Coefficient on linear adjustment to
                                          ! soil evaporation to account for non-calculation of energy balance
...
!print '("Time1 = ",f9.5," seconds.")',finish-start
  ...
  !    soil_frac_clay = 15.0
    ...
    do n = start, finish
      ...
      ! of...
      soil_frac_sand(1) = met(20,n) ! top soil sand percentage
      soil_frac_sand(2:nos_soil_layers) = met(21,n)! bot
      soil_frac_clay(1) = met(22,n)! top soil clay percentage
      soil_frac_clay(2:nos_soil_layers) = met(23,n)! bot
      call saxton_parameters(soil_frac_clay,soil_frac_sand)
      call initialise_soils(soil_frac_clay,soil_frac_sand)
      ...
      ! Potential latent energy (kgH2O.m-2.day-1)
  ...
  subroutine acm_et(wetcanopy_evap,transpiration,soilevap)
    ...
    ! - Jones 1992 p.110. partial_molar_vol_water
