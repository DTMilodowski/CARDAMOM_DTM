...
!    endif
    ...
    if ((EDC1 == 1 .or. DIAG == 1) .and. (tmp > 43.76895 .or. tmp < 10.82105)) then
...
!    ts_length = ((sum(deltat)/(nodays)) * sec_in_day) / sec_in_hour
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. sum(M_FLUXES(1:nodays,17)) < 1.0) then
  ...
  subroutine EDC1_GSI(PARS, npars, meantemp, meanrad, EDC1)
    ...
    ! temperate (max = 4.2 %)
    if ((EDC1 == 1 .or. DIAG == 1) .and. pars(30) > (pars(33)*0.125)) then
       EDC1 = 0 ; EDCD%PASSFAIL(7) = 0
    endif
    ! also apply to initial conditions
    if ((EDC1 == 1 .or. DIAG == 1) .and. pars(18) > (pars(21)*0.125)) then
       EDC1 = 0 ; EDCD%PASSFAIL(8) = 0
    endif
    ...
    if ((EDC1 == 1 .or. DIAG == 1) .and. ((pars(31)/pars(17)) > 1.5)) then
       EDC1 = 0 ; EDCD%PASSFAIL(11) = 0
    endif
...
!                        +disturbance_loss_from_litter(1:nodays))
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. (1/(torlab*365.25)) > 18.0 ) then
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. (sum(mean_ratio)/dble(nodays-sum(hak))) > 0.08 ) then
    ...
    if ((EDC2 == 1 .or. DIAG == 1) .and. ( ((mean_pools(3)/mean_pools(2)) < 0.04) .or. & 
                                           ((mean_pools(3)/mean_pools(2)) > 4.07) ) ) then
...
!    end if ! EDC .or. DIAG condition
    ...
    ! now check 
    if ((EDC2 == 1 .or. DIAG == 1) .and. ( (sum(mean_annual_pools)/dble(no_years)) > 1200.0 ) ) then
