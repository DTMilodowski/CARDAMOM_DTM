...
if (fileisthere==1){
	...
	Wtot=0;for (np=0;np<PARS.nsol;np++){if (PARS.prob[np]>Pmax-log(1000)){Wtot+=exp(PARS.prob[np]-Pmax);}}
	printf("Wtot = %e\n",Wtot);

	for (np=0;np<PARS.nsol;np++){
		/*RUNNING DALEC*/
	
		if (PARS.prob[np]>Pmax-log(1000)){
		...
		W=exp(PARS.prob[np]-Pmax);


		/*************store/write all data here***********/
		/*fluxes*/
		for (m=0;m<D.nodays;m++){
		NEE[n*D.nodays+m]+=M_NEE[m]*W/Wtot;
		RSO[n*D.nodays+m]+=M_FLUXES[m*16+13]*W/Wtot;
		LAI[n*D.nodays+m]+=M_LAI[m]*W/Wtot;
		GPP[n*D.nodays+m]+=M_FLUXES[m*16]*W/Wtot;
		NPP[n*D.nodays+m]+=(M_FLUXES[m*16]-M_FLUXES[m*16+2])*W/Wtot;

		/*pools*/
		for (p=0;p<npoo;p++){POO[p*D.nodays*NPX+n*D.nodays+m]+=log(M_POOLS[(m+1)*npoo+p])*W/Wtot;}}
