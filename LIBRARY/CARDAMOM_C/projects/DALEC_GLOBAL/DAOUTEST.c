...
if (fileisthere==1){
	...
	for (np=0;np<PARS.nsol;np++){
		...
		meannee[np]=0;meangpp[np]=0;meannpp[np]=0;meanpoo[np]=0;		
		...
		/*pools*/
		for (p=0;p<npoo;p++){POO[p*D.nodays*NPX+n*D.nodays+m]+=M_POOLS[(m+1)*npoo+p+((ND+1)*npoo*ND)]*W/Wtot;}}
...
fwrite(NEEMU,sizeof(double),NPX,neefd);fclose(neemfd);
fwrite(GPPMU,sizeof(double),NPX,gppfd);fclose(gppmfd);
fwrite(NPPMU,sizeof(double),NPX,nppfd);fclose(nppmfd);
fwrite(POOMU,sizeof(double),NPX*npoo,poofd);fclose(poomfd);
